...
Documento de handoff para retomar trabajo en otra máquina o en una conversación nueva con Claude. Última actualización: **2026-05-10 (PM noche, Windows)** — **Auditoría sistemática + fix de hubs responsive**: 6 pantallas con GridView auditadas, 3 con problema de scroll obligado por `childAspectRatio` fijo (Gomería + Logística + main_panel del chofer). Las 3 ahora usan LayoutBuilder con ratio dinámico — los tiles llenan el alto disponible y se adaptan a cualquier tamaño de pantalla sin scrollear. Bump **1.0.40+43** publicado en el primer `release_completo.ps1` exitoso end-to-end del repo (sin fallar a mitad como las veces anteriores).
